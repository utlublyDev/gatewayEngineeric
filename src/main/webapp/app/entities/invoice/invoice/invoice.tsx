  ...
  return (
    <div>
      <h2 id="invoice-heading" data-cy="InvoiceHeading">
        <Translate contentKey="gatewayEngineericApp.invoiceInvoice.home.title">Invoices</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="gatewayEngineericApp.invoiceInvoice.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gatewayEngineericApp.invoiceInvoice.home.createLabel">Create new Invoice</Translate>
          </Link>
        </div>
      </h2>
        ...
        {invoiceList && invoiceList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="gatewayEngineericApp.invoiceInvoice.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('userId')}>
                  <Translate contentKey="gatewayEngineericApp.invoiceInvoice.userId">User Id</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="gatewayEngineericApp.invoiceInvoice.code">Code</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('date')}>
                  <Translate contentKey="gatewayEngineericApp.invoiceInvoice.date">Date</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('details')}>
                  <Translate contentKey="gatewayEngineericApp.invoiceInvoice.details">Details</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('status')}>
                  <Translate contentKey="gatewayEngineericApp.invoiceInvoice.status">Status</Translate> <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {invoiceList.map((invoice, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{invoice.userStoreOwnerId}</td>
                  <td>{invoice.userId}</td>
                  <td>{invoice.code}</td>
                  <td>{invoice.date ? <TextFormat type="date" value={invoice.date} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{invoice.details}</td>
                  <td>
                    <Translate contentKey={`gatewayEngineericApp.InvoiceStatus.${invoice.status}`} />
                  </td>
                  <td>
                    <Translate contentKey={`gatewayEngineericApp.PaymentMethod.${invoice.paymentMethod}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gatewayEngineericApp.invoiceInvoice.home.notFound">No Invoices found</Translate>
            </div>
          )
        )}
      </div>
