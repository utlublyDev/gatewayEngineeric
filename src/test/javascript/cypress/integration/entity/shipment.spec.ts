...
describe('Shipment e2e test', () => {
  ...
  const shipmentSample = { userStoreOwnerId: 'Generic Refined', userId: 'navigating payment withdrawal', date: '2022-02-03T15:03:11.719Z' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {
        userStoreOwnerId: 'generation',
        userId: 'Tasty port',
        code: 'Plastic PCI contingency',
        date: '2022-02-03T14:05:55.423Z',
        details: 'Associate Uganda',
        status: 'ISSUED',
        paymentMethod: 'CASH_ON_DELIVERY',
        paymentDate: '2022-02-03T11:14:55.215Z',
        paymentAmount: 55826,
      },
    }).then(({ body }) => {
      invoice = body;
  ...
  describe('new Shipment page', () => {
    ...
    it('should create an instance of Shipment', () => {
      cy.get(`[data-cy="userStoreOwnerId"]`).type('Borders Intelligent pixel').should('have.value', 'Borders Intelligent pixel');

      cy.get(`[data-cy="userId"]`).type('withdrawal ivory').should('have.value', 'withdrawal ivory');

      cy.get(`[data-cy="trackingCode"]`).type('Account').should('have.value', 'Account');

      cy.get(`[data-cy="date"]`).type('2022-02-02T20:09').should('have.value', '2022-02-02T20:09');

      cy.get(`[data-cy="details"]`).type('virtual').should('have.value', 'virtual');
