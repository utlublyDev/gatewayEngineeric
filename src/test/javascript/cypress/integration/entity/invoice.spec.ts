...
describe('Invoice e2e test', () => {
  ...
  const invoiceSample = {
    userStoreOwnerId: 'deposit Soap',
    userId: 'Re-contextualized SDR',
    code: 'Branding network',
    date: '2022-02-02T23:35:51.801Z',
    status: 'ISSUED',
    paymentMethod: 'CASH_ON_DELIVERY',
    paymentDate: '2022-02-03T04:20:34.996Z',
    paymentAmount: 98268,
  };
  ...
  describe('new Invoice page', () => {
    ...
    it('should create an instance of Invoice', () => {
      cy.get(`[data-cy="userStoreOwnerId"]`).type('Universal Central').should('have.value', 'Universal Central');

      cy.get(`[data-cy="userId"]`).type('info-mediaries Avon').should('have.value', 'info-mediaries Avon');

      cy.get(`[data-cy="code"]`).type('SQL indigo matrix').should('have.value', 'SQL indigo matrix');

      cy.get(`[data-cy="date"]`).type('2022-02-03T06:39').should('have.value', '2022-02-03T06:39');

      cy.get(`[data-cy="details"]`).type('bleeding-edge connect Kip').should('have.value', 'bleeding-edge connect Kip');

      cy.get(`[data-cy="status"]`).select('CANCELLED');

      cy.get(`[data-cy="paymentMethod"]`).select('CREDIT_CARD');

      cy.get(`[data-cy="paymentDate"]`).type('2022-02-03T04:05').should('have.value', '2022-02-03T04:05');

      cy.get(`[data-cy="paymentAmount"]`).type('35978').should('have.value', '35978');
