...
describe('ProductCategory e2e test', () => {
  ...
  describe('new ProductCategory page', () => {
    ...
    it('should create an instance of ProductCategory', () => {
      cy.get(`[data-cy="userStoreOwnerId"]`).type('paradigm Intelligent').should('have.value', 'paradigm Intelligent');

      cy.get(`[data-cy="productCategoryName"]`).type('Metal Soap Account').should('have.value', 'Metal Soap Account');

      cy.get(`[data-cy="productCategoryNameInArabic"]`).type('Books').should('have.value', 'Books');

      cy.get(`[data-cy="productCategoryDescription"]`).type('Polarised').should('have.value', 'Polarised');

      cy.get(`[data-cy="productCategoryDescriptionInArabic"]`).type('Kansas 1080p Shoes').should('have.value', 'Kansas 1080p Shoes');

      cy.get(`[data-cy="sortOrder"]`).type('47338').should('have.value', '47338');
